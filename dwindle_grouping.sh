...
# HELPER FUNCTIONS
active_ws() {
    monitors=$(hyprctl monitors -j | jq -r '.[] | "\(.activeWorkspace) \(.specialWorkspace)"')

    normal_id=$(echo $monitors | awk '{print $1}' | jq -r '.id') 
    special_id=$(echo $monitors | awk '{print $2}' | jq -r '.id') 
...
# MAIN SCRIPT
group_direction=${1:-right} # set default if no argument is provided (for debugging)
...
echo "Looping through windows on workspace $focused_ws: $windows"    
